...
export default function EntryModal({
  ...
  const [activeReaction, setActiveReaction] = useState<ReactionKind | null>(null);
    ...
    >
      ...
      >
        <div className="flex items-start justify-between">
          ...
          <button onClick={onClose} className="rounded-full border border-white/10 px-3 py-2 text-xs hover:bg-white/10">
            Close
          </button>
        </div>

        <div className="mt-4 space-y-4 text-sm text-white/75">
          {entry.note && <div className="rounded-xl border border-white/10 p-4">{entry.note}</div>}

          {entry.imageUrl && (
            <button type="button" onClick={() => onImageClick(entry.imageUrl!)} className="overflow-hidden rounded-xl border border-white/10">
              <img src={entry.imageUrl} alt="entry" className="h-56 w-full object-cover" />
            </button>
          )}
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(reactionConfig).map(([kind, config]) => {
                ...
                return (
                  ...
                  >
                    <Icon className={`h-3.5 w-3.5 ${count > 0 ? "fill-current" : ""}`} />
                    {config.label}
          ...
          <div className="rounded-xl border border-white/10 p-4">
            ...
            <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
              {threaded.length === 0 && <p className="text-xs text-white/40">No comments yet</p>}
              {threaded.map((comment) => (
                <div key={comment.id} className="rounded-lg border border-white/10 p-3">
                  <div className={`${newActivityIds.includes(comment.id) ? "bg-rose-200/10" : ""} rounded-md px-2 py-1`}>
                    <p className="text-xs text-white/80">{comment.content}</p>
                    <p className="mt-1 text-[10px] text-white/45">
                      {comment.actor?.loveName || comment.actor?.name} · {new Date(comment.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                    </p>
                  </div>
                  ...
                  <div className="ml-4 mt-2 space-y-2 border-l border-white/15 pl-3">
                    {comment.replies.map((reply) => (
                      <div key={reply.id} className="rounded-md border border-white/10 bg-white/5 px-2 py-1.5 text-xs">
                        <p>{reply.content}</p>
                        <p className="mt-1 text-[10px] text-white/45">{reply.actor?.loveName || reply.actor?.name}</p>
                      </div>
                    ))}
            ...
            {!isJudge && (
              <div className="mt-3 flex gap-2">
                ...
                />
                <button type="button" onClick={submitComment} className="rounded-lg border border-white/15 px-3 py-2 text-xs">
                  Comment
                </button>
              </div>
            )}
          ...
          {!hasDetails(entry) && <p className="text-center text-xs text-white/50">No extra details</p>}
        </div>
